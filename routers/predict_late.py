"""
predict_late.py — Router for late-shipment prediction (AWS/ECS)
...
Response
- JSON: {"late_prediction": 0 or 1}

Notes
- No local file I/O: all scaler/encoders/model objects are provided via app.state.
- Keep this router unprefixed so the final path is exactly /predict_late/.
"""
...
logger = logging.getLogger("app.predict_late")
router = APIRouter()


@router.post("/predict_late/", tags=["prediction"])
async def predict_late(shipment_features: ShipmentFeatures, request: Request):
    logger.info("Received request to /predict_late/")
    ...
    try:
        ...
        late_model = request.app.state.late_model
        
        logger.info("Retrieved artifacts from app.state: scaler, onehot, ordinal, late_model")
    ...
    logger.info(f"Incoming payload: {shipment_features.dict()}")
    ...
    logger.info(f"Prediction generated: {int(is_late)}")
